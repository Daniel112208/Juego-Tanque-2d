...
CONSTANTES
**************/
const BARRA = 32;   // ← se mantiene, ya no se usará para disparar
/**************
OBJETOS
**************/
...
const caratula=()=>{
	...
	imagen.onload =()=>{
		game.ctx.drawImage(imagen,0,0, 650, 450);
	};
};
const seleccionar=(e)=>{
	if(game.caratula){
...
const colisiones = () => {
	game.enemigos_array.forEach((enemigo, i) => {
		game.balas_array.forEach((bala, j) => {
			if (enemigo != null && bala != null) {
				const dx = bala.x - enemigo.x;
				const dy = bala.y - enemigo.y;
				const distancia = Math.sqrt(dx * dx + dy * dy);

				if (distancia < enemigo.r + bala.w / 2) {
...
const gameOver=()=>{
	limpiarCanvas();
	mensaje("Fin del juego",0,150,"bold 80px Orbitron","#39ff14");
	mensaje(`Obtuviste ${game.puntos} puntos`,0,300,"bold 40px Orbitron","#39ff14");
	game.finJuego = true;
	sonidos.fin.play();
};
const verificar = () =>{
	// se mantiene la verificación por barra, pero ya no se usará;
	// el disparo ahora se gestiona con clic derecho (ver listener contextmenu)
	if(game.tecla_array[BARRA]){
		// No se dispara con barra, simplemente ignoramos esa tecla
...
/* NUEVO: DISPARO Y REINICIO CON CLIC DERECHO */
document.addEventListener("contextmenu", function(e){
	e.preventDefault();  // Bloquea el menú contextual
	
	/* Reiniciar si ya terminó el juego */
	if(game.finJuego){
		reiniciarJuego();
		return;
	}
	
	/* Iniciar juego desde carátula */
	if(game.caratula){
		seleccionar(e);
		return;
	}

	/* Disparo con clic derecho durante la partida */
	if(!game.caratula && !game.finJuego && game.balas>0){
...
window.onload=function(){
	...
	if(game.canvas && game.canvas.getContext){
		...
		if (game.ctx) {
			...
			caratula();
			// (click izquierdo original para seleccionar)
